...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleButtonClick(event) {
        ...
        } else if (button.tagName === 'A' && button.hasAttribute('download')) {
            // Real download link — allow default behavior and give brief feedback
            showNotification('Downloading CV...', 'success');
            // don't prevent default; browser will handle the download
        } else if (button.textContent.includes('Download CV')) {
            // Handle legacy button scenario (kept for safety)
    ...
    function smoothScroll(event) {
        ...
        // resolve URL relative ke current location
        const linkUrl = new URL(href, window.location.href);

        // jika menuju halaman lain -> navigasi biasa (biarkan server/Browser buka halaman)
        if (linkUrl.pathname !== window.location.pathname) {
            window.location.href = href;
            return;
        }

        // jika hash di halaman yang sama -> scroll smooth ke element
        const targetId = linkUrl.hash || null;
        if (targetId) {
            const targetElement = document.querySelector(targetId);
            if (targetElement) {
                const headerHeight = header?.offsetHeight || 80;
    ...
    // Initialize Active Navigation from URL (works across pages)
    function initializeActiveNav() {
        const currentPath = window.location.pathname.split('/').pop() || 'indek.html';
        ...
        navLinks.forEach(link => {
            ...
            try {
                ...
                // same page link (no hash) or page matches current
                if (linkPath === currentPath) {
                    // prefer exact hash match, otherwise mark home if no hash
                ...
                // also handle anchors that point to this page without explicit filename
                if (!linkPath && linkHash && linkHash === currentHash) {
                    link.classList.add('active');
                }
            } catch (e) {
                // ignore invalid URLs
